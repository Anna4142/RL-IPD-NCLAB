...
def calculate_transition_probabilities(data_list):
    """Calculates transition probabilities from ChoiceCounts data with memory length 2."""
    transition_counts = np.zeros((16, 4))  # 16x4 matrix for (CC, CC), (CC, CD), ..., (DD, DD) to CC, CD, DC, DD
    state_to_index = {'CC': 0, 'CD': 1, 'DC': 2, 'DD': 3}
    index_to_state = ['CC', 'CD', 'DC', 'DD']

    # Convert data_list into transitions
    transitions = []
    for data in data_list:
        for i in range(len(data) - 2):
            prev_state_1 = ''.join(data[i])
            prev_state_2 = ''.join(data[i + 1])
            current_state = ''.join(data[i + 2])
            transitions.append(((prev_state_1, prev_state_2), current_state))

    # Count the transitions
    for (prev_state_1, prev_state_2), current_state in transitions:
        if prev_state_1 in state_to_index and prev_state_2 in state_to_index and current_state in state_to_index:
            prev_index = state_to_index[prev_state_1] * 4 + state_to_index[prev_state_2]
            current_index = state_to_index[current_state]
            transition_counts[prev_index, current_index] += 1
...
def plot_transition_probabilities(stats, save_dir):
    ...
    for agent, agent_data in stats.items():
        ...
        for experiment, transition_probs in agent_data.items():
            ...
            for i, prev_state in enumerate([f'{s1},{s2}' for s1 in state_labels for s2 in state_labels]):
...
def main():
    ...
    save_directory = r"C:\Users\anush\OneDrive\study\BEN ENGLEHARD LAB\courses\technion courses\year 2 semester one\AI AND ROBOTICS\RL-IPD-NCLAB\results_across_runs\state_dist_last_2"
